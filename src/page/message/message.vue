...
export default {
  ...
  created: function() {
    ...
    if(this.storeId == '' || this.storeId == null || this.storeId == undefined){
       this.$store.state.storeId = this.$utils.getUrlKey("storeId");
    }
    this.checkStore();
  },
  computed:{
    ...
    getInfo:function(){ //获取首次银行卡信息以及头部餐厅名称
      this.$ajax({
          ...
          if(response.data.code == 200){
            ...
            this.storeName = _model.firstShop.fsshopname;
            this.onlyOneShop = _model.onlyOneShop;
...
<style scoped>
@import '../moneyQuery/moneyQuery.css'
</style>
